...
{
    [ExecuteAlways]
    public class MeshSurfaceTester : MonoBehaviour
    {
        ...
        {
            WaterProjectSettings.Instance = null;
            RenderPipelineManager.beginCameraRendering += MakeSurface;
        ...
        {
            if (camera.cameraType == CameraType.Preview || !debugCam) return;
    ...
    public class MeshSurface
    {
        ...
        public static void Render(ref Material material, ref Camera camera, ref NativeArray<float4x4> tiles, int count, int layer)
        {
            // Rendering
        ...
        private struct BaseLayout : IJob
        {
            ...
            public void Execute()
            {
                ...
                {
                    Debug.LogError($"Too many base tiles({estCount} vs. {Output.Capacity}), reduce the area of water or raise the base tile size.");
                }
                else
                {
